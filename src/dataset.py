...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # part 2 and 3
        masked_content_len = int(np.clip(trunc * np.random.normal(0.25, 0.1), 1, trunc - 2)) 
        masked_content_idx = np.random.randint(1, trunc - masked_content_len)
        masked_content = doc[masked_content_idx:masked_content_idx + masked_content_len]
        masked_content = doc[:masked_content_idx] + self.MASK_CHAR + doc[masked_content_idx + masked_content_len:] + self.MASK_CHAR + masked_content 
        masked_content += self.PAD_CHAR * (self.block_size - len(masked_content)) 

        # part 4
        input = masked_content[:-1]
        output = masked_content[1:]

        # part 5
        inp = torch.tensor([self.stoi[c] for c in input], dtype=torch.long)
        out = torch.tensor([self.stoi[c] for c in output], dtype=torch.long)
