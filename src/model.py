...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.C = torch.zeros(1, config.bottleneck_dim, config.n_embd).to(device)
    ...
    def forward(self, x_input):
        ...
        """
        x_input = self.attn(x_input, self.ln1(self.C)) + self.C
        x_input = self.mlp(self.ln2(x_input)) + x_input
        return x_input
